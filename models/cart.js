...
class Cart {
    ...
    _setLatestCartItem(item, serverData) {
        ...
        for (let sku of serverData) {
            if (item.skuId === sku.id) {
                removed = false
                delete sku.category_id
                delete sku.root_category_id
                item.sku = sku
                break
            }
        }
    ...
    getCheckedItems() {
        const cartData = this._getCartData()
        return cartData.items.filter(item => item.checked)
    }
